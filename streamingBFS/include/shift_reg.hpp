...
class ShiftReg {
 ...
 public:
  ...
  void ShiftRight(T &in) {
    // UnrolledLoop<(depth - 1)>([&](auto i) {
    //   registers_[i+1] = registers_[i];
    // });
    registers_[shift_amt] = in;
  }
  ...
  template <int shift_amt>
  T Get() {
    static_assert(shift_amt < depth);
    return registers_[shift_amt];
  }
  int position =0;
