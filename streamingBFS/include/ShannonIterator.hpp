...
class ShannonIterator {
 ...
 public:
  ...
  ShannonIterator(CounterType start, CounterType end) : end_(end) {
    ...
    fpga_tools::UnrolledLoop<0, kCountShiftRegisterSize>([&](auto i) {
      if (increment) {
    ...
    fpga_tools::UnrolledLoop<0, kInRangeShiftRegisterSize>([&](auto i) {
  ...
  bool Step() {
    ...
    fpga_tools::UnrolledLoop<0, kInRangeShiftRegisterSize - 1>([&](auto i) {
    ...
    // shift the counters
    fpga_tools::UnrolledLoop<0, shift_register_size - 1>([&](auto i) {
 ...
 private:
  ...
  fpga_tools::NTuple<CounterType,shift_register_size> counter_;

  // the shift register of in range checks
  fpga_tools::NTuple<bool,kInRangeShiftRegisterSize> inrange_;
