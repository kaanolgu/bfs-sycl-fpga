...
event &e1,event&e2,
        ...
        auto e =q.single_task<class LevelGenerator<krnl_id>>( {e1,e2}, [=]() [[intel::kernel_args_restrict]] {
...
event parallel_pipegen_kernel(queue &q,event &e1,event&e2,
                                int no_of_nodes,
                                unsigned int *usm_pipe,
                                unsigned int *d_over,
                                MyUint1 *usm_updating_mask,
        ...
        auto e =q.single_task<class PipeGenerator>( {e1,e2}, [=]() [[intel::kernel_args_restrict]] {
          int iter = 0;
          [[intel::initiation_interval(1)]]
...
template <int unroll_factor>
event maskremove_kernel(queue &q,event &e1,event &e2,event &e3,
                                int no_of_nodes,
                                MyUint1 *usm_updating_mask
                                 ){
        ...
        auto e =q.single_task<class MaskRemove>({e1,e2,e3}, [=]() [[intel::kernel_args_restrict]] {
          #pragma unroll 16
          // [[intel::initiation_interval(1)]]
          for(int tid =0; tid < no_of_nodes; tid++){
            unsigned int condition = usm_updating_mask[tid];
...
void run_bfs_fpga(int numCols, 
 ...
 auto prop_list =
  ...
  try {
    ...
    e_visited_update_0 =parallel_levelgen_kernel<0>(q,e1_1,e1_2, 0,numCols/2,usm_dist,usm_pipe, d_over,usm_updating_mask,usm_visited,global_level);
    e_visited_update_1 =parallel_levelgen_kernel<1>(q,e1_1,e1_2,numCols/2,numCols,usm_dist,usm_pipe, d_over,usm_updating_mask,usm_visited,global_level);


    e_remove_0 =parallel_pipegen_kernel<8>(q,e1_1,e1_2,numCols,usm_pipe, d_over,usm_updating_mask,usm_visited,global_level);



    e_remove_1 =maskremove_kernel<8>(q,e_remove_0,e_visited_update_1,e_visited_update_0,numCols,usm_updating_mask);
